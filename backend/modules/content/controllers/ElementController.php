...
use backend\modules\content\models\ContentToCategory;
...
class ElementController extends Controller
{
	...
	public function actionIndex(): string
	{
		...
		$dataProvider = $searchModel->search($params, [Content::TYPE_ELEMENT]);
			...
			if ($model->load($post) && $model->validate($post)) {
				...
				if ($model->save()) {
					if (!empty($model->category_id) && is_array($model->category_id)) {
						$categories = ContentToCategory::find()->where(['content_id' => $model->id])->all();
						foreach ($categories as $category) {
							$category->delete();
						}
						foreach ($model->category_id as $item) {
							$category = ContentCategory::find()->where(['id' => $item])->one();
							if (!empty($category)) {
								$associate = new ContentToCategory();
								$associate->content_id = $model->id;
								$associate->category_id = $category->id;
								$associate->save();
							}
						}
					}
	...
	{
		if (($model = Content::find()->where(['id' => $id, 'type_id' => Content::TYPE_ELEMENT])->one()) !== null) {
			return $model;
		}
