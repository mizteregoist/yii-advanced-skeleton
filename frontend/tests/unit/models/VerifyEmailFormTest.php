...
use backend\modules\user\forms\VerifyEmailForm;
...
class VerifyEmailFormTest extends \Codeception\Test\Unit
{
    ...
    public function testVerifyCorrectToken()
    {
        ...
        expect($user)->isInstanceOf('backend\modules\user\models\User');

        expect($user->username)->equals('test.test');
        expect($user->email)->equals('test@mail.com');
        expect($user->status)->equals(\backend\modules\user\models\User::STATUS_ACTIVE);
